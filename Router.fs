...
let updateArrived (token : string) (ctx : UpdateContext) =
    let bot data = api token data |> Async.RunSynchronously |> processResult
    let botResult data = api token data |> Async.RunSynchronously
